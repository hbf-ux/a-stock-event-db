...
export default function Home() {
  const [view, setView] = useState<"announcements" | "events" | "reviews">("announcements");
  ...
  const loadAll = useCallback(async () => {
    try {
      const [eventResponse, announcementResponse, healthResponse, reviewResponse] = await Promise.all([
        fetch("/api/events", { cache: "no-store" }),
        fetch("/api/announcements", { cache: "no-store" }),
        fetch("/api/health", { cache: "no-store" }),
        fetch("/api/reviews", { cache: "no-store" }),
      ]);
      if (!eventResponse.ok || !announcementResponse.ok || !healthResponse.ok || !reviewResponse.ok) throw new Error("API unavailable");
  ...
  const pageTitle = view === "announcements" ? "公告中心" : view === "reviews" ? "人工审核" : "股权质押";
  const pageDescription = view === "announcements" ? "真实官方公告已入库；待解析公告保留在审核队列。" : view === "reviews" ? "补全自动解析缺失字段，审核操作全程留痕。" : "仅展示字段校验通过的结构化质押事件。";
  const pageEyebrow = view === "announcements" ? "OFFICIAL ANNOUNCEMENTS" : view === "reviews" ? "REVIEW QUEUE" : "PLEDGE EVENTS";
  ...
  return <main className="shell">
    <aside className="sidebar">
      ...
      <div className="sideBottom"><button><i>⚙</i>系统设置</button><div className="system"><span></span><div><b>{state === "online" ? "数据库在线" : state === "loading" ? "正在连接" : "连接异常"}</b><small>D1 · R2 · Worker API</small></div></div></div>
    </aside>
    <section className="content">
      ...
      <div className="page">
        ...
        {view === "reviews" ? <ReviewPanel reviews={reviews} onOpen={openReview} /> : <section className="panel">
