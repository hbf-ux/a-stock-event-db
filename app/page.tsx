...
function BackfillPlanPanel() {
  ...
  return <section className="backfillPlan panel"><div className="panelHead"><div><h2>历史回补计划</h2><span>先识别公告日期缺口，再决定是否回补</span></div><span className="tag extra">最多 31 天</span></div><div className="backfillControls"><label>开始日期<input type="date" value={start} onChange={(e) => setStart(e.target.value)} /></label><label>结束日期<input type="date" value={end} onChange={(e) => setEnd(e.target.value)} /></label><button className="secondary" disabled={loading} onClick={() => void inspect()}>{loading ? "检查中…" : "检查缺口"}</button></div>{result && <div className="backfillResult"><b>{result.missingDates?.length || 0} 个日期没有已入库公告</b><div className="backfillDates">{(result.missingDates || []).map((date) => <span key={date}>{date}</span>)}</div><small>{result.scope}</small></div>}</section>;
}
...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {view === "dashboard" && <><CoverageOverview coverage={coverage} /><BackfillPlanPanel /></>}
