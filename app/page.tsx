...
export default function Home() {
  const [view, setView] = useState<"announcements" | "events">("announcements");
  ...
  const loadAll = useCallback(async () => {
    try {
      const [eventResponse, announcementResponse, healthResponse] = await Promise.all([
        fetch("/api/events", { cache: "no-store" }),
        fetch("/api/announcements", { cache: "no-store" }),
        fetch("/api/health", { cache: "no-store" }),
      ]);
      if (!eventResponse.ok || !announcementResponse.ok || !healthResponse.ok) throw new Error("API unavailable");
