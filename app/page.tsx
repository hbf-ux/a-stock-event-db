...
type EventRow = {
  ...
  amount: string; ratio: string; total: string; type: string; source: string; id: string;
};
...
export default function Home() {
  ...
  const exportCsv = () => {
    const header = "公告日期,股票代码,股票简称,股东名称,质权人,质押数量,占其持股,占总股本,事件类型,来源\n";
    const body = filtered.map(r => [r.date,r.code,r.name,r.shareholder,r.pledgee,r.amount,r.ratio,r.total,r.type,r.source].map(v => `"${v}"`).join(",")).join("\n");
    const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob(["\ufeff" + header + body], {type:"text/csv"}));
    a.download = `A股股权质押_${date || "全部"}.csv`; a.click(); URL.revokeObjectURL(a.href); flash(`已导出 ${filtered.length} 条记录`);
  };
  ...
  return (
    <main className="shell">
      <aside className="sidebar">
        ...
        <div className="sideBottom"><button onClick={()=>flash("系统运行正常，数据源连接稳定")}><i>⚙</i> 系统设置</button><div className="system"><span></span><div><b>系统运行正常</b><small>最后同步 07-24 18:32</small></div></div></div>
      </aside>
      ...
      <section className="content">
        ...
        <div className="page">
          <div className="titleRow"><div><p className="eyebrow">PLEDGE EVENTS</p><h1>股权质押</h1><p>基于官方公告的股权质押事件结构化数据，每条记录均可追溯。</p></div><div className="actions"><button className="secondary" onClick={()=>flash("已发起同步任务")}>↻&nbsp; 同步公告</button><button className="primary" onClick={exportCsv}>⇩&nbsp; 导出数据</button></div></div>
          ...
          <section className="panel">
            ...
            <div className="resultMeta">共找到 <b>{filtered.length}</b> 条记录 <span>数据来源：巨潮资讯、交易所官方披露</span></div>
            <div className="tableWrap"><table><thead><tr><th>公告日期</th><th>股票</th><th>股东名称</th><th>质权人</th><th className="num">质押数量</th><th className="num">占其持股</th><th className="num">占总股本</th><th>事件类型</th><th>来源</th><th></th></tr></thead><tbody>
              {filtered.map(r=><tr key={r.id}><td className="mono">{r.date}</td><td><b>{r.name}</b><small>{r.code}</small></td><td title={r.shareholder}>{r.shareholder}</td><td title={r.pledgee}>{r.pledgee}</td><td className="num mono">{r.amount}</td><td className="num mono">{r.ratio}</td><td className="num mono">{r.total}</td><td><span className={`tag ${r.type.includes("解除")?"release":r.type.includes("补充")?"extra":"new"}`}>{r.type}</span></td><td><span className="sourceDot"></span>{r.source}</td><td><button className="detail" onClick={()=>setSelected(r)}>查看 ›</button></td></tr>)}
      ...
      {selected&&<div className="overlay" onClick={()=>setSelected(null)}><aside className="drawer" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setSelected(null)}>×</button><p className="eyebrow">EVENT TRACE</p><h2>事件溯源详情</h2><div className="stockTitle"><div>{selected.code.slice(0,2)}</div><span><b>{selected.name}</b><small>{selected.code} · {selected.source}</small></span><span className="tag new">{selected.type}</span></div><dl><dt>公告日期</dt><dd>{selected.date}</dd><dt>股东名称</dt><dd>{selected.shareholder}</dd><dt>质权人</dt><dd>{selected.pledgee}</dd><dt>质押数量</dt><dd>{selected.amount}</dd><dt>占其持股 / 总股本</dt><dd>{selected.ratio} / {selected.total}</dd></dl><div className="trace"><h3>数据血缘</h3><p><i>1</i><span><b>官方公告采集</b><small>{selected.source} · 原始 PDF</small></span></p><p><i>2</i><span><b>结构化解析</b><small>解析器 v1.0.3 · 置信度 99.2%</small></span></p><p><i>3</i><span><b>完整性校验</b><small>MD5 去重 · 字段校验通过</small></span></p></div><div className="hash"><span>公告编号</span><b>{selected.id}</b><span>文件哈希</span><code>8f24c2d7…a093</code></div><button className="primary full" onClick={()=>flash("原始公告链接将在数据接口接入后开放")}>查看原始公告 ↗</button></aside></div>}
