...
function StockCoveragePanel({ onProfile }: { onProfile: (stock: string) => void }) {
  ...
  return <section className="stockCoverage panel"><div className="panelHead"><div><h2>公司级历史覆盖</h2><span>优先发现公告少、历史短或解析不足的股票</span></div><span className="tag extra">按覆盖薄弱排序</span></div><div className="tableWrap"><table><thead><tr><th>股票</th><th>公告范围</th><th>公告数</th><th>已解析</th><th>事件范围</th><th>事件数</th></tr></thead><tbody>{rows.map((row) => <tr key={row.stockCode}><td><button className="stockLink" onClick={() => onProfile(row.stockCode)}><b>{row.stockName}</b><small>{row.stockCode}</small></button></td><td className="mono">{row.firstDate || "—"} 至 {row.latestDate || "—"}</td><td className="mono">{row.announcements}</td><td className={row.parsedAnnouncements < row.announcements ? "warnValue mono" : "safeValue mono"}>{row.parsedAnnouncements}/{row.announcements}</td><td className="mono">{row.eventFirstDate || "—"} 至 {row.eventLatestDate || "—"}</td><td className="mono">{row.events}</td></tr>)}{!rows.length && <tr><td colSpan={6} className="empty">正在加载公司覆盖数据</td></tr>}</tbody></table></div><p className="researchDisclaimer">公告少不代表公司没有质押；可能是历史尚未回补、公告来源缺失或仍在待解析队列。点击股票可进入公司画像。</p></section>;
}
