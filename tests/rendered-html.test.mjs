...
test("public product is a daily closing report backed by the production workflow", async () => {
  ...
  assert.doesNotMatch(worker, /await generateDailyReportArtifacts/);
  assert.match(worker, /delivery:"client_generated_png"/);
...
test("international daily report and existing billing backend remain wired", async()=>{
  ...
  assert.match(english,/ONE DAY · ONE VERIFIED IMAGE/);
...
test("deployment bundle exists", async () => {
  ...
  assert.doesNotMatch(cronWorker,/outside-production-window/);
  assert.match(cronWorker,/return \{ shouldRun: true/);
