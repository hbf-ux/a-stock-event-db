...
interface Env {
  ...
  /** OpenAI review is opt-in. Rules and manual review remain the default path. */
  OPENAI_REVIEW_ENABLED?: string;
...
async function ensureSchema(db: D1Database) {
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS announcement (announcement_id TEXT PRIMARY KEY, stock_code TEXT NOT NULL, stock_name TEXT NOT NULL, title TEXT NOT NULL, announce_date TEXT NOT NULL, report_date TEXT NOT NULL, pdf_url TEXT, r2_key TEXT, source TEXT NOT NULL, crawl_time TEXT NOT NULL, md5 TEXT NOT NULL UNIQUE, sha256 TEXT, parse_status TEXT NOT NULL DEFAULT 'pending', parse_attempts INTEGER NOT NULL DEFAULT 0, last_error TEXT)`,
  ...
  await db.prepare("UPDATE announcement SET parse_status=CASE WHEN parse_attempts>=2 THEN 'review' ELSE 'queued' END,last_error=COALESCE(last_error,'解析任务中断，已自动恢复') WHERE parse_status='processing' AND announcement_id IN (SELECT entity_id FROM audit_log WHERE entity_type='announcement' AND action='parse_claim' GROUP BY entity_id HAVING MAX(julianday(created_at))<julianday('now','-5 minutes'))").run();
...
async function seed(db: D1Database) {
  ...
  for (const r of demo) {
    ...
    statements.push(db.prepare("INSERT OR IGNORE INTO announcement (announcement_id,stock_code,stock_name,title,announce_date,report_date,pdf_url,source,crawl_time,md5,sha256,parse_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(r[0],r[1],r[2],r[3],r[4],r[4],r[5],r[6],now,r[7],r[7],"parsed"));
...
async function processAnnouncement(db: D1Database, documents: R2Bucket, id: string, env?: Env, options: {forceOpenAI?:boolean;allowFreshVersionAttempt?:boolean} = {}) {
  ...
  const openaiEnabled = Boolean(env?.OPENAI_API_KEY) && (options.forceOpenAI === true || env?.OPENAI_REVIEW_ENABLED?.trim().toLowerCase() === "true");
  let openaiMeta: {model:string;responseId:string;requestId:string;usage:OpenAIUsage|null;durationMs:number} | null = null;
  let openaiError = "";
  const quotaState = localIncomplete && openaiEnabled && options.forceOpenAI !== true
  ...
  const openaiAllowed = openaiEnabled && !quotaBlocked && ((item.parseAttempts || 0) < 3 || options.forceOpenAI === true || options.allowFreshVersionAttempt === true);
  ...
  if (!completeRows.length || rows.some((row) => row.missing.length > 0)) {
    ...
    const reviewReason = openaiAttempted
      ...
      : quotaBlocked
        ...
        : `本地规则解析仍缺少字段：${missing.join("、")}；已自动转入人工审核`;
...
async function processPendingQueue(db: D1Database, documents: R2Bucket, requestedLimit = 3, env?: Env, priorityDate?: string) {
  ...
  const pending = priorityDate
    ? await db.prepare("SELECT announcement_id AS id FROM announcement WHERE parse_status IN ('queued','archived') AND report_date=? ORDER BY announcement_id LIMIT ?").bind(priorityDate,limit).all<{id:string}>()
  ...
  for (const row of pending.results) {
    ...
    const claim=await db.prepare("UPDATE announcement SET parse_status='processing',parse_attempts=parse_attempts+1,last_error=NULL WHERE announcement_id=? AND parse_status IN ('queued','archived')").bind(row.id).run();
    if(!claim.meta.changes)continue;
    await db.prepare("INSERT INTO audit_log (entity_type,entity_id,action,after_json,actor,created_at) VALUES (?,?,?,?,?,?)").bind("announcement",row.id,"parse_claim",JSON.stringify({runId:run?.id,priorityDate:priorityDate||null}),"pdf-worker",claimedAt).run();
...
async function ingestCninfo(db: D1Database, date: string) {
  ...
  for (const item of announcements) {
    ...
    await db.batch([
      ...
      db.prepare("INSERT INTO announcement (announcement_id,stock_code,stock_name,title,announce_date,report_date,pdf_url,source,crawl_time,md5,parse_status) VALUES (?,?,?,?,?,?,?,?,?,?,?)").bind(item.announcementId,item.secCode,item.secName,title,toDate(item.announcementTime),reportDateForDisclosure(item.announcementTime),pdfUrl,"巨潮资讯",now,`pending:${item.announcementId}`,"queued"),
...
async function promoteExchangeObservation(db:D1Database,documents:R2Bucket,id:number,viewer:string):Promise<PromotionResult> {
  ...
  await db.batch([
    ...
    db.prepare("INSERT INTO announcement (announcement_id,stock_code,stock_name,title,announce_date,report_date,pdf_url,r2_key,source,crawl_time,md5,sha256,parse_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(announcementId,row.stockCode,row.stockName,row.title,row.announceDate,row.announceDate,pdfUrl.toString(),r2Key,row.source,now,sha256,sha256,"archived"),
...
type AutomaticSyncState = {
  enabled: boolean;
  status: "disabled"|"fresh"|"running"|"started"|"published";
  targetDate: string;
...
async function publishDailyReport(env:Env,date:string,viewer:string,notes="",publicationKind:"provisional"|"final"="final") {
  ...
  return {ok:true as const,date,status:savedStatus,publicationKind,pendingCount,publishedAt:now,reportVersion:version,delivery:"client_generated_png"};
}
...
async function api(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (url.pathname === "/api/health") {
    ...
    const openaiEnabled=Boolean(env.OPENAI_API_KEY)&&env.OPENAI_REVIEW_ENABLED?.trim().toLowerCase()==="true";
    return json({ status: "ok", storage: { d1: true, r2: true }, scheduler:{handlerReady:true,secureHttpTriggerConfigured:Boolean(env.PRODUCTION_CRON_SECRET),productionTargetDate:automaticProduction.targetDate}, automaticSync:automaticProduction, automaticMaintenance, automatedReview: { configured: Boolean(env.OPENAI_API_KEY), enabled:openaiEnabled, available: openaiEnabled && !quotaBlocked, mode: openaiEnabled ? "rules-then-openai" : "rules-then-manual", model: openaiEnabled ? (env.OPENAI_OCR_MODEL || "gpt-5.6-luna") : null, circuitState:openaiEnabled?(quotaBlocked?String(lastError?.category||"blocked"):"closed"):"disabled", blockedUntil: openaiEnabled&&quotaBlocked ? quotaState?.value : null, quotaBlockedUntil:openaiEnabled&&quotaBlocked?quotaState?.value:null, lastError,lastErrorAt:openaiErrorState?.updatedAt||null,maxAutomaticAttempts: openaiEnabled?2:0,usage24h:openaiUsage||{} }, billing:{provider:"stripe",configured:Boolean(env.STRIPE_SECRET_KEY&&env.STRIPE_WEBHOOK_SECRET&&Object.values(stripePlanPrices(env)).every(Boolean)),mode:env.STRIPE_SECRET_KEY?.startsWith("sk_live_")?"live":env.STRIPE_SECRET_KEY?"test":"disabled"}, stats, timestamp: new Date().toISOString() });
  }
  ...
  if(url.pathname==="/api/maintenance"&&request.method==="GET"){
    ...
    return json({lastResult:state?JSON.parse(state.value):null,lastUpdatedAt:state?.updatedAt||null,candidates,latestEvidenceRun,scope:"每天最多用六字段规则解析器v2.5重试3份待审核公告、处理5份PDF证据并向前回补1个交易日"});
  }
  ...
  if (url.pathname === "/api/reviews" && request.method === "GET") {
    ...
    const [result,existing,counts]=await Promise.all([
      ...
      env.DB.prepare("SELECT SUM(CASE WHEN r.status='pending' AND a.parse_status='review' THEN 1 ELSE 0 END) AS pending,SUM(CASE WHEN r.status='pending' AND a.parse_status IN ('queued','archived','processing') THEN 1 ELSE 0 END) AS processing,SUM(CASE WHEN r.status='approved' AND COALESCE(r.reviewer,'') NOT IN ('worker','openai') THEN 1 ELSE 0 END) AS approved,SUM(CASE WHEN r.status='rejected' THEN 1 ELSE 0 END) AS rejected FROM review_queue r JOIN announcement a ON a.announcement_id=r.announcement_id WHERE a.report_date=?").bind(reportDate).first(),
    ]);
