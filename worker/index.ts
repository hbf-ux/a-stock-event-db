...
async function api(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if(url.pathname==="/api/daily-report"&&request.method==="GET"){
    ...
    const version=Number((snapshot.published as {reportVersion?:number}|null)?.reportVersion||1);let artifacts=["published","provisional"].includes(snapshot.status)?await dailyArtifactLinks(env,date,version):{png:null,pdf:null,locked:false};let artifactGenerationError:string|null=null;
    if(["published","provisional"].includes(snapshot.status)&&!artifacts.locked){try{await generateDailyReportArtifacts(env,date,version,snapshot.status==="published"?"final":"provisional",Number((snapshot.announcement as {pending?:number}).pending||0));artifacts=await dailyArtifactLinks(env,date,version);}catch(error){artifactGenerationError=error instanceof Error?error.message:"artifact generation failed";}}
    return json({...snapshot,events:events.results,automaticProduction,automatedReview:{configured:Boolean(env.OPENAI_API_KEY),available:Boolean(env.OPENAI_API_KEY)&&!blockedUntil,blockedUntil,lastError,lastErrorAt:openaiErrorState?.updatedAt||null,usage:openaiUsage||{calls:0,successes:0,inputTokens:0,outputTokens:0,totalTokens:0,cachedInputTokens:0}},artifacts,artifactGenerationError,clock:{shanghaiDate:shanghaiDate(),productionTargetDate:automaticProduction.targetDate,cutoffHour:20,publicationDeadlineHour:21},methodology:"仅收录新增质押，解除质押、解除再质押、补充质押及展期全部排除；每日20:00固定当日日报范围；20:00后的官方公告顺延到下一自然日日报；后台持续补齐并在三所差异清零后升级最终版",deliverables:{image:"PNG仅包含股票名称、股票代码、质押股东、质权人、质押股票数量、质押日期",pdf:"与PNG使用同一份锁定数据及相同六字段生成"},generatedAt:new Date().toISOString()});
  }
